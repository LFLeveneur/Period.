            ...
            {days.map((day, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Dots Container — affiche les indicateurs d'état des séances */}
                  <div className="absolute bottom-2 flex gap-1">
                    {/* Dot vert si séances prévues complétées */}
                    {dayData?.pendingSession.some(s => s.completedHistoryId) && (
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-sm shadow-emerald-500/20" />
                    )}
                    {/* Dot gris si séances prévues non complétées */}
                    {dayData?.pendingSession.some(s => !s.completedHistoryId) && (
                      <div className="w-1.5 h-1.5 rounded-full bg-slate-300 shadow-sm shadow-slate-300/20" />
                    )}
                    {/* Dot bleu si séances bonus/extra */}
                    {dayData?.extraSessions.length! > 0 && (
                      <div className="w-1.5 h-1.5 rounded-full bg-blue-400 shadow-sm shadow-blue-400/20" />
                    )}
                  </div>
                </button>
              );
              ...
              <div className="space-y-4 mb-8">
                ...
                {selectedDay.pendingSession.map(session => (
                  <button
                    ...
                    onClick={() => {
                      setSelectedDay(null);
                      if (session.completedHistoryId) {
                        navigate(`/history/${session.completedHistoryId}`);
                      } else {
                ...
                {/* Aucune séance */}
                {selectedDay.pendingSession.length === 0 && selectedDay.extraSessions.length === 0 && (
